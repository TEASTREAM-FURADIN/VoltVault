...
const RadarChart = ({ memos, userSettings }) => {
  const data = MainCategories.map(cat => ({
    name: cat, 
    count: memos.filter(m => (userSettings.genres[m.genre]?.group || 'その他') === cat).length 
  }));
        ...
        <div className="p-3 flex flex-wrap gap-2 bg-slate-900 border-t border-slate-700">
          {tags.map(t => {
            ...
            return (
              ...
              ><DynamicIcon name={t.icon} size={12} /> {t.key}</button>
            )
          })}
        </div>
      )}
...
const MarkupModalCanvas = ({ markupModal, setMarkupModal, formData, setFormData }) => {
  ...
  const handleSaveImage = () => {
    ...
    // ★ 保存時に、テキストの周りに「半透明の黒背景＋色付き枠線」を付けて保存する
    ctx.globalCompositeOperation = 'source-over';
        ...
        <main className="p-4 max-w-xl mx-auto">
          ...
          {view === 'settings' && (
            <div className="space-y-6 pb-10 animate-in slide-in-from-right">
              ...
              <div className="text-center py-4 opacity-30"><Gamepad2 size={32} className="mx-auto text-cyan-600 mb-2"/><p className="text-[10px] font-black text-cyan-600 uppercase tracking-widest">ELECTRIC CLIPPER MASTER v2.7</p></div>
            </div>
          )}
